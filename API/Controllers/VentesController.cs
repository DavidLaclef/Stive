...
public class VentesController : ControllerBase
{
    ...
    public VentesController(StiveContext context)
    {
        _context = context;
    }
    ...
    // GET: api/Ventes
    [HttpGet]
    public async Task<ActionResult<IEnumerable<VenteLightDto>>> GetVente()
    {
    ...
    public async Task<ActionResult<VenteDto>> GetVente(int id)
    {
        var vente = await _context.Vente
            .Include(v => v.ListeMouvementStock).ThenInclude(liste => liste.Produit)
            .Include(v => v.Client)
            .Where(v => v.Id == id)
            .Select(v => v.ToDto())
            .FirstOrDefaultAsync();
